...
if "notif" not in st.session_state:
    st.session_state.notif = {k: None for k in ["playlist","browser","koperasi","reservasi","rs","editor","bank","kasir","queue_impl","kop_queue"]}
else:
    for k in ["kasir","queue_impl","kop_queue"]:
        if k not in st.session_state.notif:
            st.session_state.notif[k] = None

# SK7 – Kasir Minimarket
if "kasir_antrian" not in st.session_state:
    st.session_state.kasir_antrian = []
if "kasir_dilayani" not in st.session_state:
    st.session_state.kasir_dilayani = None

# SK8 – Implementasi Queue
if "queue_impl" not in st.session_state:
    st.session_state.queue_impl = []

# SK9 – Antrian Koperasi
if "kop_queue" not in st.session_state:
    st.session_state.kop_queue = []
if "kop_current" not in st.session_state:
    st.session_state.kop_current = None
...
<div class="header-utama">
    ...
    <p>Studi Kasus 1–9 · Playlist · Browser History · Koperasi · Hotel · Rumah Sakit · Undo/Redo · Kasir · Queue · Antrian Koperasi</p>
</div>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
# TAB 7 – KASIR MINIMARKET
# ══════════════════════════════════════════════

with tab7:
    st.markdown("""
    <div style="background:linear-gradient(135deg,#713f12,#d97706);border-radius:12px;
         padding:1.2rem 1.5rem;margin-bottom:1rem;color:white">
        <h2 style="margin:0;font-size:1.3rem">🛒 Simulasi Antrian Kasir Minimarket</h2>
        <p style="margin:0.1rem 0 0;opacity:0.75;font-size:0.82rem">Implementasi Queue menggunakan collections.deque</p>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">🧑 Pelanggan Baru</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Enqueue</span>', unsafe_allow_html=True)
        nama_kasir = st.text_input("Nama Pelanggan", placeholder="Masukkan nama...", key="in_kasir")
        if st.button("🛒 Masuk Antrian (Enqueue)", use_container_width=True, type="primary", key="btn_enq_kasir"):
            if nama_kasir.strip():
                st.session_state.kasir_antrian.append(nama_kasir.strip())
                set_notif("kasir", "sukses", f"{nama_kasir} berhasil masuk ke antrian kasir.")
                st.rerun()
            else:
                set_notif("kasir", "gagal", "Masukkan nama pelanggan.")
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">💰 Kasir</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Dequeue</span>', unsafe_allow_html=True)
        if st.button("💳 Layani Pelanggan (Dequeue)", use_container_width=True,
                     disabled=len(st.session_state.kasir_antrian) == 0, key="btn_deq_kasir"):
            st.session_state.kasir_dilayani = st.session_state.kasir_antrian.pop(0)
            set_notif("kasir", "info", f"Melayani: {st.session_state.kasir_dilayani}")
            st.rerun()

        if st.session_state.kasir_dilayani:
            st.markdown(f"""
            <div class="layanan-aktif">
                <div style="font-size:0.75rem;color:#4ade80;letter-spacing:1px">🟢 SEDANG DILAYANI</div>
                <div style="font-size:1.8rem;font-weight:900;color:#15803d">{st.session_state.kasir_dilayani}</div>
            </div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="stat-box" style="margin-bottom:1rem">
            <div class="stat-angka">{len(st.session_state.kasir_antrian)}</div>
            <div class="stat-label">Pelanggan Menunggu</div>
        </div>""", unsafe_allow_html=True)

        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">📋 Daftar Antrian Kasir</div>', unsafe_allow_html=True)
        if st.session_state.kasir_antrian:
            for i, p in enumerate(st.session_state.kasir_antrian, 1):
                panah = "→" if i < len(st.session_state.kasir_antrian) else "→ NULL"
                st.markdown(f'<div class="item-list"><span class="nomor">{i}.</span><span style="flex:1">{p}</span><span style="color:#cbd5e1;font-size:0.75rem">{panah}</span></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="kosong">Antrian kosong</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)


# ══════════════════════════════════════════════
# TAB 8 – IMPLEMENTASI QUEUE
# ══════════════════════════════════════════════
...
with tab8:
    ...
    <div style="background:linear-gradient(135deg,#1e3a5f,#2563eb);border-radius:12px;
         padding:1.2rem 1.5rem;margin-bottom:1rem;color:white">
        <h2 style="margin:0;font-size:1.3rem">📋 Implementasi Queue</h2>
        <p style="margin:0.1rem 0 0;opacity:0.75;font-size:0.82rem">Operasi dasar Enqueue dan Dequeue</p>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">➕ Tambah ke Queue</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Enqueue</span>', unsafe_allow_html=True)
        nama_q = st.text_input("Masukkan Nama", placeholder="Nama...", key="in_queue_impl")
        if st.button("➕ Enqueue", use_container_width=True, type="primary", key="btn_enqueue_impl"):
            if nama_q.strip():
                st.session_state.queue_impl.append(nama_q.strip())
                set_notif("queue_impl", "sukses", f"{nama_q} berhasil ditambahkan ke antrian.")
                st.rerun()
            else:
                set_notif("queue_impl", "gagal", "Nama tidak boleh kosong.")
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">➖ Keluarkan dari Queue</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Dequeue</span>', unsafe_allow_html=True)
        if st.button("➖ Dequeue", use_container_width=True,
                     disabled=len(st.session_state.queue_impl) == 0, key="btn_dequeue_impl"):
            keluar = st.session_state.queue_impl.pop(0)
            set_notif("queue_impl", "info", f"{keluar} keluar dari antrian.")
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

    with col2:
        notif_html("queue_impl")
        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">📊 Isi Antrian</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Visualisasi Queue</span>', unsafe_allow_html=True)
        if st.session_state.queue_impl:
            for i, item in enumerate(st.session_state.queue_impl, 1):
                panah = "→" if i < len(st.session_state.queue_impl) else "→ NULL"
                label = " (FRONT)" if i == 1 else (" (REAR)" if i == len(st.session_state.queue_impl) else "")
                st.markdown(f'<div class="item-list"><span class="nomor">{i}.</span><span style="flex:1">{item}<span style="color:#6366f1;font-size:0.75rem"> {label}</span></span><span style="color:#cbd5e1;font-size:0.75rem">{panah}</span></div>', unsafe_allow_html=True)
            st.markdown(f"<small style='color:#94a3b8'>Total: <b>{len(st.session_state.queue_impl)}</b> item</small>", unsafe_allow_html=True)
        else:
            st.markdown('<div class="kosong">Antrian kosong</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
# TAB 9 – ANTRIAN PELANGGAN KOPERASI
# ══════════════════════════════════════════════

with tab9:
    st.markdown("""
    <div style="background:linear-gradient(135deg,#14532d,#16a34a);border-radius:12px;
         padding:1.2rem 1.5rem;margin-bottom:1rem;color:white">
        <h2 style="margin:0;font-size:1.3rem">🏢 Antrian Pelanggan Koperasi</h2>
        <p style="margin:0.1rem 0 0;opacity:0.75;font-size:0.82rem">Implementasi Queue menggunakan collections.deque</p>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">🧑 Tambah Pelanggan</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Enqueue</span>', unsafe_allow_html=True)
        nama_kop_q = st.text_input("Nama Pelanggan", placeholder="Nama lengkap...", key="in_kop_queue")
        if st.button("✅ Masuk Antrian", use_container_width=True, type="primary", key="btn_enq_kop"):
            if nama_kop_q.strip():
                st.session_state.kop_queue.append(nama_kop_q.strip())
                set_notif("kop_queue", "sukses", f"{nama_kop_q} masuk antrian koperasi.")
                st.rerun()
            else:
                set_notif("kop_queue", "gagal", "Masukkan nama pelanggan.")
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">🏪 Layanan</div>', unsafe_allow_html=True)
        st.markdown('<span class="badge">Operasi: Dequeue</span>', unsafe_allow_html=True)
        if st.button("🔔 Layani Pelanggan Berikutnya", use_container_width=True,
                     disabled=len(st.session_state.kop_queue) == 0, key="btn_deq_kop"):
            st.session_state.kop_current = st.session_state.kop_queue.pop(0)
            set_notif("kop_queue", "info", f"Melayani: {st.session_state.kop_current}")
            st.rerun()

        if st.session_state.kop_current:
            st.markdown(f"""
            <div class="layanan-aktif">
                <div style="font-size:0.75rem;color:#4ade80;letter-spacing:1px">🟢 SEDANG DILAYANI</div>
                <div style="font-size:1.6rem;font-weight:900;color:#15803d;margin-top:0.3rem">{st.session_state.kop_current}</div>
            </div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with col2:
        notif_html("kop_queue")
        st.markdown(f"""
        <div class="stat-box" style="margin-bottom:1rem">
            <div class="stat-angka">{len(st.session_state.kop_queue)}</div>
            <div class="stat-label">Jumlah Antrian</div>
        </div>""", unsafe_allow_html=True)

        st.markdown('<div class="kartu">', unsafe_allow_html=True)
        st.markdown('<div class="kartu-judul">📋 Daftar Antrian</div>', unsafe_allow_html=True)
        if st.session_state.kop_queue:
            for i, nama in enumerate(st.session_state.kop_queue, 1):
                panah = "→" if i < len(st.session_state.kop_queue) else "→ NULL"
                st.markdown(f'<div class="item-list"><span class="nomor">{i}.</span><span style="flex:1">{nama}</span><span style="color:#cbd5e1;font-size:0.75rem">{panah}</span></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="kosong">Belum ada pelanggan dalam antrian</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
